...
mod ts {
    ...
    // use deno_webgpu_get_declaration from the external context
    ...
    #[op2]
    #[serde]
    // using the same op that is used in `src.rs` for loading modules and reading
    ...
    fn op_load(
        ...
        // we need a basic file to send to src to warm it up.
    ...
    pub fn create_compiler_snapshot(snapshot_path: PathBuf, cwd: &Path) {
        ...
        // NOTE(bartlomieju): Compressing the snapshot in debug build took
...
fn main() {
    ...
    let skip_cross_check = env::var("DENO_SKIP_CROSS_BUILD_CHECK").map_or(false, |v| v == "1");
