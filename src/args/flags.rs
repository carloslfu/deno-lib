...
impl PermissionFlags {
    ...
    pub fn to_options(&self, cli_arg_urls: &[Cow<Url>]) -> PermissionsOptions {
        fn handle_allow<T: Default>(allow_all: bool, value: Option<T>) -> Option<T> {
            if allow_all {
        ...
        runtime_args(cmd, true, false, true)
            ...
            .arg(
                Arg::new("include")
                    .long("include")
                    .help(cstr!(
                        "Includes an additional module in the compiled executable's module graph.
        ...
        cmd.arg(config_arg())
            ...
            .arg(
                Arg::new("ext")
                    ...
                    .value_parser([
                        "ts", "tsx", "js", "jsx", "md", "json", "jsonc", "css", "scss", "sass",
                        "less", "html", "svelte", "vue", "astro", "yml", "yaml", "ipynb",
                    ])
                    .help_heading(FMT_HEADING)
                    .requires("files"),
            )
...
fn init_subcommand() -> Command {
    ...
    .defer(|cmd| {
        cmd.arg(Arg::new("dir").value_hint(ValueHint::DirPath))
            .arg(
                Arg::new("lib")
                    .long("lib")
...
fn install_parse(flags: &mut Flags, matches: &mut ArgMatches) -> clap::error::Result<()> {
    ...
    if matches.get_flag("entrypoint") {
        ...
        flags.subcommand = DenoSubcommand::Install(InstallFlags {
            kind: InstallKind::Local(InstallFlagsLocal::Entrypoints(entrypoints.collect())),
        });
...
mod tests {
    ...
    fn repl_strace_ops() {
        ...
        let r = flags_from_vec(svec!["deno", "repl", "--strace-ops=http,websocket"]);
        assert_eq!(
            r.unwrap().strace_ops,
    ...
    fn test_no_clear_watch_flag_without_watch_flag() {
        ...
        assert!(
            &error_message.contains("error: the following required arguments were not provided:")
        );
    ...
    fn remove_subcommand() {
        ...
        let r = flags_from_vec(svec!["deno", "remove", "@david/which", "@luca/hello"]);
        assert_eq!(
            r.unwrap(),
            Flags {
                subcommand: DenoSubcommand::Remove(RemoveFlags {
