...
pub fn ts_config_to_transpile_and_emit_options(
    ...
    let options: deno_config::deno_json::EmitConfigOptions =
        serde_json::from_value(config.0).context("Failed to parse compilerOptions")?;
...
fn discover_npmrc(
    ...
    fn try_to_read_npmrc(dir: &Path) -> Result<Option<(String, PathBuf)>, NpmRcLoadError> {
    ...
    fn try_to_parse_npmrc(source: String, path: &Path) -> Result<Arc<ResolvedNpmRc>, AnyError> {
        let npmrc = NpmRc::parse(&source, &get_env_var)
            .with_context(|| format!("Failed to parse .npmrc at {}", path.display()))?;
        let resolved = npmrc
...
impl CliOptions {
    pub fn new(
        ...
        let deno_dir_provider = Arc::new(DenoDirProvider::new(flags.internal.cache_path.clone()));
    ...
    pub fn from_flags(flags: Arc<Flags>) -> Result<Self, AnyError> {
        ...
        let resolve_workspace_discover_options = || {
            ...
            let config_parse_options = deno_config::deno_json::ConfigParseOptions::default();
        ...
        let resolve_empty_options = || WorkspaceDirectoryEmptyOptions {
            root_dir: Arc::new(ModuleSpecifier::from_directory_path(&initial_cwd).unwrap()),
            use_vendor_dir: maybe_vendor_override.unwrap_or(VendorEnablement::Disable),
        };
        ...
        let start_dir = match &flags.config_flag {
            ...
            ConfigFlag::Disabled => WorkspaceDirectory::empty(resolve_empty_options()),
        };
    ...
    pub fn into_self_and_flags(self: Arc<CliOptions>) -> (Arc<CliOptions>, Arc<Flags>) {
    ...
    pub async fn create_workspace_resolver(
        ...
        let cli_arg_specified_import_map = if overrode_no_import_map {
            ...
            let maybe_import_map_specifier = self.resolve_specified_import_map_specifier()?;
            match maybe_import_map_specifier {
                Some(specifier) => {
                    let value = resolve_import_map_value_from_specifier(&specifier, file_fetcher)
                        .await
    ...
    pub fn node_modules_dir(&self) -> Result<Option<NodeModulesDirMode>, AnyError> {
    ...
    pub fn to_compiler_option_types(&self) -> Result<Vec<deno_graph::ReferrerImports>, AnyError> {
        self.workspace()
    ...
    pub fn resolve_deno_lint_config(&self) -> Result<deno_lint::linter::LintConfig, AnyError> {
        let ts_config_result = self.resolve_ts_config_for_emit(TsConfigType::Emit)?;

        let (transpile_options, _) =
            crate::args::ts_config_to_transpile_and_emit_options(ts_config_result.ts_config)?;
    ...
    pub fn use_byonm(&self) -> bool {
        if matches!(
            self.sub_command(),
            DenoSubcommand::Install(_) | DenoSubcommand::Add(_) | DenoSubcommand::Remove(_)
        ) {
    ...
    pub fn unstable_features(&self) -> Vec<String> {
        ...
        if !from_config_file.is_empty() {
            ...
            for unstable_value_from_config_file in &from_config_file {
                if !all_valid_unstable_flags.contains(&unstable_value_from_config_file.as_str()) {
                    log::warn!(
