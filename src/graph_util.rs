    ...
    graph
        ...
        .flat_map(|error| {
            ...
            let mut message = match &error {
                ...
                ModuleGraphError::ModuleError(error) => enhanced_integrity_error_message(error)
...
impl ModuleGraphCreator {
    ...
    pub async fn create_graph_with_loader(
        ...
        self.create_graph_with_options(CreateGraphOptions {
            is_dynamic: false,
            graph_kind,
            roots,
            loader: Some(loader),
    ...
    async fn type_check_graph(&self, graph: ModuleGraph) -> Result<Arc<ModuleGraph>, AnyError> {
        self.type_checker
            .check(
                graph,
                check::CheckOptions {
...
impl ModuleGraphBuilder {
    ...
    pub async fn build_graph_with_npm_resolution<'a>(
        ...
        self.build_graph_with_npm_resolution_and_build_options(
            graph,
            options.roots,
    ...
    async fn build_graph_with_npm_resolution_and_build_options<'a>(
        ...
        if roots.iter().any(|r| r.scheme() == "npm") && self.npm_resolver.as_byonm().is_some() {
...
pub fn enhanced_resolution_error_message(error: &ResolutionError) -> String {
    ...
    let maybe_hint = if let Some(specifier) = get_resolution_error_bare_node_specifier(error) {
        ...
        if range.specifier.scheme() == "file" {
            match error.as_ref() {
                ResolveError::Specifier(specifier_error) => {
                    if let SpecifierError::ImportPrefixMissing { specifier, .. } = specifier_error {
                        maybe_specifier = Some(specifier);
                    }
                }
