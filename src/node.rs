...
use lib_node_resolver::analyze::CjsAnalysis as ExtNodeCjsAnalysis;
use lib_node_resolver::analyze::CjsAnalysisExports;
use lib_node_resolver::analyze::CjsCodeAnalyzer;
...
pub type CliNodeCodeTranslator = NodeCodeTranslator<CliCjsCodeAnalyzer, DenoFsNodeResolverEnv>;
...
impl CliCjsCodeAnalyzer {
    ...
    async fn inner_cjs_analysis(
        ...
        if let Some(analysis) = self.cache.get_cjs_analysis(specifier.as_str(), source_hash) {
            return Ok(analysis);
        }
        ...
        let analysis = if is_maybe_cjs {
            ...
            deno_core::unsync::spawn_blocking({
                ...
                move || -> Result<_, AnyError> {
                    let parsed_source = maybe_parsed_source.map(Ok).unwrap_or_else(|| {
