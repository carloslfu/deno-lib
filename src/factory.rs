...
struct CliFactoryServices {
    ...
    lib_deno_resolver: Deferred<Arc<CliDenoResolver>>,
...
impl CliFactory {
    ...
    pub fn cli_options(&self) -> Result<&Arc<CliOptions>, AnyError> {
        self.services
            .cli_options
            .get_or_try_init(|| CliOptions::from_flags(self.flags.clone()).map(Arc::new))
    }
    ...
    pub fn in_npm_pkg_checker(&self) -> Result<&Arc<dyn InNpmPackageChecker>, AnyError> {
                    ...
                    create_cli_npm_resolver(if cli_options.use_byonm() {
                        ...
                        CliNpmResolverCreateOptions::Managed(CliManagedNpmResolverCreateOptions {
                            ...
                            npm_install_deps_provider: Arc::new(
                                NpmInstallDepsProvider::from_workspace(cli_options.workspace()),
                            ),
    ...
    pub async fn workspace_resolver(&self) -> Result<&Arc<WorkspaceResolver>, AnyError> {
        self.services
            .workspace_resolver
    ...
    pub async fn lib_deno_resolver(&self) -> Result<&Arc<CliDenoResolver>, AnyError> {
        self.services
            .lib_deno_resolver
            .get_or_try_init_async(async {
                let cli_options = self.cli_options()?;
            ...
            .get_or_try_init_async(
                async {
                    ...
                    Ok(Arc::new(CliResolver::new(CliResolverOptions {
                        ...
                        bare_node_builtins_enabled: cli_options.unstable_bare_node_builtins(),
                        lib_deno_resolver: self.lib_deno_resolver().await?.clone(),
                    })))
                }
                .boxed_local(),
            )
    ...
    pub fn emit_cache(&self) -> Result<&Arc<EmitCache>, AnyError> {
        self.services
            .emit_cache
            .get_or_try_init(|| Ok(Arc::new(EmitCache::new(self.deno_dir()?.gen_cache.clone()))))
    }
    ...
    pub fn emitter(&self) -> Result<&Arc<Emitter>, AnyError> {
        self.services.emitter.get_or_try_init(|| {
            let cli_options = self.cli_options()?;
            let ts_config_result = cli_options.resolve_ts_config_for_emit(TsConfigType::Emit)?;
    ...
    fn create_cli_main_worker_options(&self) -> Result<CliMainWorkerOptions, AnyError> {
        ...
        let create_coverage_collector = if let Some(coverage_dir) = cli_options.coverage_dir() {
            let coverage_dir = PathBuf::from(coverage_dir);
            let fn_: crate::worker::CreateCoverageCollectorCb = Box::new(move |session| {
