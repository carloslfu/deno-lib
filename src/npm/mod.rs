...
use lib_deno_resolver::npm::ByonmInNpmPackageChecker;
...
use lib_node_resolver::NpmPackageFolderResolver;
...
pub trait CliNpmResolver: NpmPackageFolderResolver + CliNpmReqResolver {
    fn into_npm_pkg_folder_resolver(self: Arc<Self>) -> Arc<dyn NpmPackageFolderResolver>;
    fn into_npm_req_resolver(self: Arc<Self>) -> Arc<dyn CliNpmReqResolver>;
    fn into_process_state_provider(self: Arc<Self>) -> Arc<dyn NpmProcessStateProvider>;
...
impl NpmFetchResolver {
    pub fn new(file_fetcher: Arc<FileFetcher>, npmrc: Arc<ResolvedNpmRc>) -> Self {
        Self {
    ...
    pub async fn package_info(&self, name: &str) -> Option<Arc<NpmPackageInfo>> {
        ...
        let fetch_package_info = || async {
            ...
            let maybe_auth_header = maybe_auth_header_for_npm_registry(registry_config).ok()?;
