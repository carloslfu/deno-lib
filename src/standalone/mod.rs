...
use lib_node_resolver::analyze::NodeCodeTranslator;
...
struct SharedModuleLoaderState {
    ...
    lib_node_resolver: Arc<NodeResolver>,
...
impl ModuleLoader for EmbeddedModuleLoader {
    fn resolve(
        ...
        if self.shared.lib_node_resolver.in_npm_package(&referrer) {
            return Ok(self
                .shared
                .lib_node_resolver
                .resolve(
        ...
        match mapped_resolution {
            ...
            | Ok(MappedResolution::ImportMap { specifier, .. }) => {
                ...
                if specifier.scheme() == "jsr" {
                    if let Some(specifier) = self.shared.modules.resolve_specifier(&specifier)? {
    ...
    fn get_host_defined_options<'s>(
        ...
        if self.shared.lib_node_resolver.in_npm_package(&name) {
    ...
    fn load(
        ...
        if self
            .shared
            .lib_node_resolver
            .in_npm_package(original_specifier)
        {
            ...
            return deno_core::ModuleLoadResponse::Async(
                async move {
                    ...
                    let code_cache_entry =
                        shared.get_code_cache(&code_source.found_url, code_source.code.as_bytes());
        ...
        match self.shared.modules.read(original_specifier) {
                                ...
                                DenoCompileModuleSource::Bytes(module_code_bytes) => {
                                    match module_code_bytes {
                                        Cow::Owned(bytes) => Cow::Owned(
                                            crate::util::text_encoding::from_utf8_lossy_owned(
                                                bytes,
                                            ),
                                        ),
            ...
            Ok(None) => deno_core::ModuleLoadResponse::Sync(Err(type_error(format!(
                "{MODULE_NOT_FOUND}: {}",
                original_specifier
            )))),
            Err(err) => deno_core::ModuleLoadResponse::Sync(Err(type_error(format!("{:?}", err)))),
        }
    }
...
impl NodeRequireLoader for EmbeddedModuleLoader {
    ...
    fn load_text_file_lossy(&self, path: &std::path::Path) -> Result<String, AnyError> {
        Ok(self.shared.fs.read_text_file_lossy_sync(path, None)?)
    }

    fn is_maybe_cjs(&self, specifier: &ModuleSpecifier) -> Result<bool, ClosestPkgJsonError> {
...
pub async fn run(data: StandaloneData) -> Result<i32, AnyError> {
    ...
    let root_dir_url = Arc::new(ModuleSpecifier::from_directory_path(&root_path).unwrap());
    ...
    let (in_npm_pkg_checker, npm_resolver) = match metadata.node_modules {
        Some(binary::NodeModules::Managed { node_modules_dir }) => {
            ...
            let in_npm_pkg_checker = create_in_npm_pkg_checker(
                CreateInNpmPkgCheckerOptions::Managed(CliManagedInNpmPkgCheckerCreateOptions {
        ...
        Some(binary::NodeModules::Byonm {
            ...
            let in_npm_pkg_checker = create_in_npm_pkg_checker(CreateInNpmPkgCheckerOptions::Byonm);
    ...
    let lib_node_resolver = Arc::new(NodeResolver::new(
        deno_runtime::deno_node::DenoFsNodeResolverEnv::new(fs.clone()),
    ...
    let npm_req_resolver = Arc::new(CliNpmReqResolver::new(NpmReqResolverOptions {
        byonm_resolver: (npm_resolver.clone()).into_maybe_byonm(),
        fs: CliDenoResolverFs(fs.clone()),
        in_npm_pkg_checker: in_npm_pkg_checker.clone(),
        lib_node_resolver: lib_node_resolver.clone(),
        npm_req_resolver: npm_resolver.clone().into_npm_req_resolver(),
    }));
    let cjs_esm_code_analyzer =
        CliCjsCodeAnalyzer::new(node_analysis_cache, cjs_tracker.clone(), fs.clone(), None);
    let node_code_translator = Arc::new(NodeCodeTranslator::new(
        cjs_esm_code_analyzer,
        deno_runtime::deno_node::DenoFsNodeResolverEnv::new(fs.clone()),
        in_npm_pkg_checker,
        lib_node_resolver.clone(),
        npm_resolver.clone().into_npm_pkg_folder_resolver(),
        pkg_json_resolver.clone(),
    ));
    ...
    let permissions = {
        let mut permissions = metadata.permissions.to_options(/* cli_arg_urls */ &[]);
        ...
        let desc_parser = Arc::new(RuntimePermissionDescriptorParser::new(fs.clone()));
