...
use lib_deno_resolver::npm::NpmReqResolverOptions;
use lib_deno_resolver::DenoResolverOptions;
use lib_deno_resolver::NodeAndNpmReqResolver;
use lib_node_resolver::errors::ClosestPkgJsonError;
use lib_node_resolver::InNpmPackageChecker;
use lib_node_resolver::NodeModuleKind;
use lib_node_resolver::NodeResolutionMode;
use std::borrow::Cow;
...
impl LspScopeResolver {
    async fn from_config_data(
                    ...
                    imports
                        ...
                        .map(|(referrer, imports)| {
                            let resolver = SingleReferrerGraphResolver {
                                ...
                                jsx_import_source_config: maybe_jsx_import_source_config.as_ref(),
                            };
        ...
        let package_json_deps_by_resolution = (|| {
            ...
            let result = dependencies
                .iter()
                .flat_map(|(name, _)| {
                    let req_ref = NpmPackageReqReference::from_str(&format!("npm:{name}")).ok()?;
    ...
    fn snapshot(&self) -> Arc<Self> {
        ...
        let npm_resolver = self.npm_resolver.as_ref().map(|r| r.clone_snapshotted());
        ...
        Arc::new(Self {
            ...
            package_json_deps_by_resolution: self.package_json_deps_by_resolution.clone(),
...
impl LspResolver {
    pub async fn from_config(
        ...
        Self {
            unscoped: Arc::new(
                LspScopeResolver::from_config_data(None, cache, http_client_provider).await,
            ),
            by_scope,
        }
    }
    ...
    pub fn as_cli_resolver(&self, file_referrer: Option<&ModuleSpecifier>) -> &CliResolver {
    ...
    pub fn npm_to_file_url(
        ...
        Some(into_specifier_and_media_type(Some(
            npm_pkg_req_resolver
                .resolve_req_reference(req_ref, referrer, referrer_kind, NodeResolutionMode::Types)
                .ok()?,
        )))
    }
    ...
    pub fn in_node_modules(&self, specifier: &ModuleSpecifier) -> bool {
        ...
        if let Some(node_resolver) = &self.get_scope_resolver(Some(specifier)).node_resolver {
    ...
    pub fn is_bare_package_json_dep(
        ...
        let Some(npm_pkg_req_resolver) = resolver.npm_pkg_req_resolver.as_ref() else {
            return false;
        };
        npm_pkg_req_resolver
    ...
    fn get_scope_resolver(&self, file_referrer: Option<&ModuleSpecifier>) -> &LspScopeResolver {
...
impl<'a> ResolverFactory<'a> {
    ...
    async fn init_npm_resolver(
        ...
        let options = if enable_byonm {
            ...
            CliNpmResolverCreateOptions::Managed(CliManagedNpmResolverCreateOptions {
                ...
                maybe_node_modules_path: self.config_data.and_then(|d| d.node_modules_dir.clone()),
    ...
    pub fn cli_resolver(&self) -> &Arc<CliResolver> {
        self.services.cli_resolver.get_or_init(|| {
            ...
            let lib_deno_resolver = Arc::new(CliDenoResolver::new(DenoResolverOptions {
                in_npm_pkg_checker: self.in_npm_pkg_checker().clone(),
                node_and_req_resolver: match (self.node_resolver(), npm_req_resolver) {
                    (Some(node_resolver), Some(npm_req_resolver)) => Some(NodeAndNpmReqResolver {
                ...
                workspace_resolver: self.config_data.map(|d| d.resolver.clone()).unwrap_or_else(
                    || {
...
impl LspIsCjsResolver {
    ...
    pub fn get_maybe_doc_module_kind(
        ...
        self.get_lsp_referrer_kind(specifier, maybe_document.and_then(|d| d.is_script()))
    }
...
impl<'a> deno_graph::source::Resolver for SingleReferrerGraphResolver<'a> {
    ...
    fn resolve(
        ...
        self.cli_resolver
            .resolve(specifier_text, referrer_range, self.referrer_kind, mode)
    }
}
    ...
    fn resolve(&self, specifier: &Url) -> Option<Url> {
        ...
        let destination = loop {
            ...
            if let Some(location) = headers.get("location") {
                ...
                let Ok(target) = deno_core::resolve_import(location, specifier.as_str()) else {
                    break None;
                };
                chain.push((
                    current.clone(),
