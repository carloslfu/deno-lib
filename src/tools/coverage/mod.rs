...
fn generate_coverage_report(
    ...
    for function in &script_coverage.functions {
        ...
        let line_index =
            range_to_src_line_index(&function.ranges[0], &text_lines, &maybe_source_map);
...
fn collect_coverages(
    ...
    for file_path in file_paths {
        let new_coverage = fs::read_to_string(file_path.as_path())
            ...
            .and_then(|json| {
                serde_json::from_str::<cdp::ScriptCoverage>(&json).map_err(AnyError::from)
            })
...
pub fn cover_files(flags: Arc<Flags>, coverage_flags: CoverageFlags) -> Result<(), AnyError> {
    ...
    let script_coverages =
        collect_coverages(cli_options, coverage_flags.files, cli_options.initial_cwd())?;
