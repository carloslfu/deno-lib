...
impl<TGraphContainer: ModuleGraphContainer> CliModuleLoaderInner<TGraphContainer> {
    ...
    fn resolve_referrer(&self, referrer: &str) -> Result<ModuleSpecifier, AnyError> {
        ...
        if deno_core::specifier_has_uri_scheme(referrer) {
            ...
            deno_core::resolve_path(referrer, &self.shared.initial_cwd).map_err(|e| e.into())
        } else {
    ...
    fn inner_resolve(
        ...
        let specifier = match graph.get(&specifier) {
            ...
            Some(Module::External(module)) => node::resolve_specifier_into_node_modules(
                &module.specifier,
        ...
        match graph.get(specifier) {
            ...
            Some(deno_graph::Module::Js(JsModule {
                ...
                let code: ModuleCodeString = match media_type {
                    ...
                    MediaType::TypeScript | MediaType::Mts | MediaType::Jsx | MediaType::Tsx => {
    ...
    async fn load_maybe_cjs(
        ...
        let js_source = if media_type.is_emittable() {
            Cow::Owned(
                self.emitter
                    .emit_parsed_source(specifier, media_type, ModuleKind::Cjs, original_source)
                    .await?,
            )
        } else {
        ...
        Ok(ModuleCodeStringSource {
            code: match text {
                ...
                // the borrowed text
                Cow::Borrowed(_) => ModuleSourceCode::String(original_source.clone().into()),
...
impl<TGraphContainer: ModuleGraphContainer> ModuleLoader for CliModuleLoader<TGraphContainer> {
    ...
    fn prepare_load(
        ...
        async move {
            ...
            module_load_preparer
                .prepare_module_load(graph, &[specifier], is_dynamic, lib, permissions, None)
                .await?;
    ...
    fn load_text_file_lossy(&self, path: &Path) -> Result<String, AnyError> {
        ...
        if media_type.is_emittable() {
            ...
            if self.in_npm_pkg_checker.in_npm_package(&specifier) {
                return Err(NotSupportedKindInNpmError {
                    media_type,
                    specifier,
                }
                .into());
